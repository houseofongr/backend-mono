...
class TreeInfoTest {
    ...
    Element element1 = Element.create(1L, 9L, 1L, null, "element1", "유니버스의 엘리먼트-1", 0.5f, 0.5f, 0.7f, 0.6f);
    Element element2 = Element.create(2L, 10L, 1L, 1L, "element2", "스페이스1의 엘리먼트-1", 0.4f, 0.2f, 0.5f, 0.1f);
    Element element3 = Element.create(3L, 11L, 1L, 3L, "element3", "스페이스3의 엘리먼트-1", 0.2f, 0.3f, 0.4f, 0.2f);
    Element element4 = Element.create(4L, 12L, 1L, 4L, "element4", "스페이스4의 엘리먼트-1", 0.7f, 0.4f, 0.3f, 0.3f);
    Element element5 = Element.create(5L, 13L, 1L, 4L, "element5", "스페이스4의 엘리먼트-2", 0.8f, 0.1f, 0.2f, 0.4f);
    Element element6 = Element.create(6L, 14L, 1L, 5L, "element6", "스페이스5의 엘리먼트-1", 0.8f, 0.1f, 0.2f, 0.4f);
    Element element7 = Element.create(7L, 15L, 1L, 5L, "element7", "스페이스5의 엘리먼트-2", 0.8f, 0.1f, 0.2f, 0.4f);
    List<Element> elements = List.of(element1, element2, element3, element4, element5, element6, element7);
    ...
    void testCreateRoot() {
        ...
        // when
        TreeInfo root = TreeInfo.create(universe, spaces, elements);
    ...
    void testDFS() {
        ...
        TreeInfo element1 = root.getComponent(Element.class, 1L).getTreeInfo();
        TreeInfo element2 = root.getComponent(Element.class, 2L).getTreeInfo();
        TreeInfo element3 = root.getComponent(Element.class, 3L).getTreeInfo();
        TreeInfo element4 = root.getComponent(Element.class, 4L).getTreeInfo();
        TreeInfo element5 = root.getComponent(Element.class, 5L).getTreeInfo();
        TreeInfo element6 = root.getComponent(Element.class, 6L).getTreeInfo();
        TreeInfo element7 = root.getComponent(Element.class, 7L).getTreeInfo();

        // then
        assertThat(root.getComponent(Space.class, 6L)).isNull();
        assertThat(root.getComponent(Element.class, 8L)).isNull();
        ...
        assertDFS(element1, Element.class, 1, 1, root);
        assertDFS(element2, Element.class, 2, 2, space1);
        assertDFS(element3, Element.class, 3, 3, space3);
        assertDFS(element4, Element.class, 4, 3, space4);
        assertDFS(element5, Element.class, 5, 3, space4);
        assertDFS(element6, Element.class, 6, 3, space5);
        assertDFS(element7, Element.class, 7, 3, space5);
    }
