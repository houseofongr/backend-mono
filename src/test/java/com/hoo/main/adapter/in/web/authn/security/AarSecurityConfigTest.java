...
class AarSecurityConfigTest {
    ...
    void testPublicURL() throws Exception {
        mockMvc.perform(get("/authn/login/{provider}", "kakao").accept(MediaType.APPLICATION_JSON))
                .andExpect(status().is(302));
        mockMvc.perform(get("/authn/login/kakao/callback", "kakao").accept(MediaType.APPLICATION_JSON))
    ...
    void testUnregisteredURL() throws Exception {
        mockMvc.perform(get("/invalid_url").accept(MediaType.APPLICATION_JSON))
