= 아카이브 오브 옹알 API 문서
Sangyeop. Nam <spearoad15@gmail.com>
:doctype: book
:description: 아카이브 오브 옹알 API 문서입니다.
:icons: font
:source-highlighter: highlightjs
:highlightjs-theme:
:toc: left
:toclevels: 3
:pdf-themesdir: {docdir}/themes
:pdf-theme: ko
:pdf-fontsdir: {docdir}/fonts
:stylesheet: stylesheet.css
:imagesdir: {docdir}/images
:stylesdir: {docdir}/styles
// :sectlinks:

아카이브 오브 옹알 공식 API 문서입니다.

This is Archive Of Ong(AOO) R Official API Documents.

[[aar-api]]
== 소리기록실(AAR) API

소리기록실(Audio Record Room; ARR) 사용자를 위한 API입니다.

[[aar-authn-flow]]
=== 소리기록실 인증 플로우
.로그인 플로우차트
//https://excalidraw.com/#json=aTnuIuf60RUxbEY_BPuum,jbkQ3CdbR9HT5byljXptkw
image::login.png[login flow]

1. `**User 로그인**`
    a. ``사용자``가 소리기록실 페이지로 접속해서 로그인을 시도합니다.
    b. ``프론트엔드``에서 ``백엔드``로 특정 서드파티 도메인의 로그인 API 요청을 전송합니다.
    c. 이를 받은 ``백엔드``는 브라우저를 ``서드파티``의 로그인 페이지로 이동시킵니다.
2. `**SNS 로그인 시도**`
    a. ``사용자``가 서드파티 페이지에서 로그인을 시도합니다.
    b. 정상 로그인일 경우, ``서드파티``에서 백엔드로 코드를 전송합니다.
    c. 해당 코드를 수신한 ``백엔드``는 토큰을 교환하여 사용자 정보를 받습니다.
3. `**SNS 로그인 완료**`
    a. ``백엔드``에서 받은 사용자 정보를 확인하여 ``프론트엔드``로 전송합니다.
    b. ``프론트엔드``에서는 해당 사용자 정보를 포함하여 메인 페이지를 전송합니다.
    c. ``사용자``는 메인 페이지로 이동합니다.

.회원가입 플로우차트
//https://excalidraw.com/#json=ZlQOr-_1yHyOU65158_Hd,h0Df7KyDrhCJoL7BzYLHqA
image::regist.png[regist flow]

1. `**User 로그인**`
    - 사용자가 로그인을 시도합니다.(로그인 플로우와 동일)
2. `**SNS 계정 등록**`
    a. ``백엔드``에서 해당 SNS 계정이 등록되어 있지 않으면 해당 계정을 **임시 저장**합니다.
    b. ``프론트엔드``에게 임시 저장된 계정 정보와 함께 **임시 토큰**을 전송합니다.
    c. ``프론트엔드``는 해당 정보를 임시 저장하고, 회원가입 페이지를 전송합니다.
3. `**회원가입**`
    a. ``사용자``는 회원가입 페이지에서 필요한 양식을 채워 회원가입을 시도합니다.
    b. ``프론트엔드``는 해당 정보를 ``백엔드``로 전송합니다.
    c. ``백엔드``는 해당 정보를 등록하고, 사용자 정보와 함께 **토큰**을 전송합니다.
    d. ``프론트엔드``는 해당 사용자 정보를 포함하여 메인 페이지를 전송합니다.
    e. ``사용자``는 메인 페이지로 이동합니다.

[[aar-authn-api]]
=== 인증(Authn) API

소리기록실의 인증을 위한 API입니다.

[[aar-authn-login-api]]
==== SNS 로그인 API

소리기록실에 SNS 로그인을 시도합니다.

- **서드파티 인증 페이지로 이동**하여 정상 인증이 완료되면 해당 계정으로 로그인합니다.

operation::aar-authn-login[snippets='http-request,path-parameters,http-response,response-headers']

> 파라미터에 대한 세부 정보는 아래 **Response fields**를 참고바랍니다.

operation::aar-authn-login-2[snippets='response-fields']

[[aar-authn-regist-api]]
==== 회원가입 API

소리기록실에 회원가입을 시도합니다.

operation::aar-authn-regist[snippets='http-request,request-fields,http-response,response-fields']

[[admin-api]]
== 관리자(Admin) API

관리자페이지에서 소리기록실을 관리하기 위한 API 입니다.

[[admin-house-api]]
=== 하우스(House) API

하우스 관리를 위한 API 입니다.

[[admin-house-post-api]]
==== 하우스 생성 API

관리자가 지정한 하우스의 좌표에 맞게 하우스를 저장합니다.

===== Http Request
include::{snippets}/admin-house-post/http-request.adoc[]

===== Request Form
include::{snippets}/admin-house-post/request-parts.adoc[]

> 위 요청에서 각 이미지 파일의 **Form 태그 name 속성값**은 아래 <<CreateHouseMetadata>>와 일치해야 합니다.

===== CreateHouseMetadata 예시
include::{snippets}/admin-create-house-metadata/response-body.adoc[]

===== CreateHouseMetadata
include::{snippets}/admin-create-house-metadata/response-fields.adoc[]

operation::admin-house-post[snippets='http-response,response-fields']

[[admin-house-get-list-api]]
==== 하우스 리스트 조회 API

여러 건의 하우스를 검색 조건에 맞게 조회합니다.

operation::admin-house-get-list[snippets='http-request,path-parameters,http-response,response-fields']

[[admin-house-get-api]]
==== 하우스 상세 조회 API

하우스와 내부 룸에 대한 상세 정보를 조회합니다.

operation::admin-house-get[snippets='http-request,path-parameters,http-response,response-fields']

[[admin-house-patch-api]]
==== 하우스 정보 수정 API

하우스의 기본 정보를 수정하는 API입니다.

> 하우스의 이미지와 좌표는 최초 생성 이후 수정할 수 없습니다.

operation::admin-house-patch[snippets='http-request,path-parameters,request-fields,http-response,response-fields']

[[admin-house-delete-api]]
==== 하우스 삭제 API

하우스를 삭제하는 API입니다.

> 하우스 내부에 포함된 룸과 아이템을 함께 삭제합니다.

operation::admin-house-delete[snippets='http-request,path-parameters,http-response,response-fields']

> 차후 사용자의 홈이 생성된 하우스는 삭제할 수 없도록 할 예정입니다.

[[admin-room-api]]
=== 룸(Room) API

룸 관리를 위한 API 입니다.

[[admin-room-get-api]]
==== 룸 정보 상세조회 API

룸의 상세 정보를 조회하는 API입니다.

> 차후 아이템 관련 정보를 추가할 예정입니다.

operation::admin-house-get-room[snippets='http-request,path-parameters,http-response,response-fields']

[[admin-room-patch-api]]
==== 룸 정보 수정 API

룸의 기본 정보를 수정하는 API입니다.

> 룸의 이미지와 좌표는 최초 생성 이후 수정할 수 없습니다.

operation::admin-house-patch-room[snippets='http-request,request-fields,http-response,response-fields']

[[admin-room-delete-api]]
==== 룸 삭제 API

하우스의 룸을 삭제하는 API입니다.

> 룸 내부에 포함된 아이템을 함께 삭제합니다.

operation::admin-house-delete-room[snippets='http-request,path-parameters,http-response,response-fields']

> 차후 사용자의 음원이 담긴 룸은 삭제할 수 없도록 할 예정입니다.

[[admin-user-api]]
=== 사용자(User) API

소리기록실 사용자 정보 관리를 위한 API 입니다.

[[admin-user-get-list-api]]
==== 사용자 조회 API

소리기록실 사용자를 조회하는 API 입니다.

operation::admin-user-get-list[snippets='http-request,path-parameters,http-response,response-fields']

[[admin-user-get-home-list-api]]
==== 사용자 홈 조회 API

사용자가 보유한 홈을 조회하는 API 입니다.

operation::admin-user-get-home-list[snippets='http-request,path-parameters,http-response,response-fields']

[[admin-home-api]]
=== 홈(Home) API

소리기록실의 홈 정보 관리를 위한 API 입니다.

[[admin-home-post-api]]
==== 홈 생성 API

홈을 생성하는 API 입니다.

operation::admin-home-post[snippets='http-request,request-fields,http-response,response-fields']

[[admin-home-get-api]]
==== 홈 조회 API

홈 정보를 조회하는 API 입니다.

operation::admin-home-get[snippets='http-request,path-parameters,http-response,response-fields']

[[admin-home-delete-api]]
==== 홈 삭제 API

홈을 삭제하는 API 입니다.

> 차후 홈 내부에 아이템이 존재할 경우 함께 삭제할 예정입니다.

operation::admin-home-delete[snippets='http-request,path-parameters,http-response,response-fields']

[[admin-item-api]]
=== 아이템(item) API

[[admin-item-post-api]]
==== 아이템 생성 API

관리자가 지정한 아이템의 좌표와 음원에 맞게 아이템을 저장합니다.

===== Http Request
include::{snippets}/admin-item-post/http-request.adoc[]

===== Request Form
include::{snippets}/admin-item-post/request-parts.adoc[]

> 위 요청에서 각 음원 파일의 **Form 태그 name 속성값**은 아래 <<CreateItemMetadata>>의 **form 필드와 일치**해야 합니다.

===== CreateItemMetadata 예시
include::{snippets}/admin-create-item-metadata/response-body.adoc[]

===== CreateItemMetadata
include::{snippets}/admin-create-item-metadata/response-fields.adoc[]


operation::admin-item-post[snippets='http-response,response-fields']


[[file-api]]
== 파일(File) API

파일서버의 업로드, 다운로드와 관련된 API입니다.

[[file-public-api]]
=== 공개(Public) API

별도의 권한 없이 요청 가능한 API입니다.

[[file-public-images-post-api]]
==== 이미지 업로드 API

이미지 파일을 공개 서버에 업로드합니다.

operation::file-public-images-upload[snippets='http-request,http-response,response-fields']

[[file-public-images-get-api]]
==== 이미지 로드 API

이미지 파일을 공개 서버로부터 로드합니다.

operation::file-public-images-download[snippets='http-request,path-parameters,response-file']

[[file-private-api]]
=== 비공개(Private) API

해당 API는 **PRIVATE_FILE_ACCESS** 권한이 필요합니다.

> 차후 해당 권한을 바탕으로 자원 접근을 제어할 예정입니다.

[[file-private-images-get-api]]
==== 이미지 로드 API

이미지 파일을 비공개 서버로부터 로드합니다.

operation::file-private-images-download[snippets='http-request,path-parameters,response-file']

[[file-private-audios-get-api]]
==== 음원 로드 API

음원 파일을 비공개 서버로부터 로드합니다.

operation::file-private-audios-download[snippets='http-request,path-parameters,response-file']

[[error-code]]
== 에러코드

[[error-code-API]]
=== 에러코드 API

API 요청 간 발생하는 에러코드를 조회할 수 있습니다.

> **에러코드 조회 API**의 응답 결과는 아래 <<에러코드 테이블>>을 참고 바랍니다.

[[error-code-aar-api]]
==== 소리기록실 에러코드 API

소리기록실에서 관리하는 에러코드를 확인할 수 있습니다.

include::{snippets}/aar-error-code/http-request.adoc[]

[[error-code-admin-api]]
==== 관리자 에러코드 API

관리자 페이지에서 관리하는 에러코드를 확인할 수 있습니다.

include::{snippets}/admin-error-code/http-request.adoc[]


[[error-code-file-api]]
==== 파일 에러코드 API

파일 서버에서 관리하는 에러코드를 확인할 수 있습니다.

include::{snippets}/file-error-code/http-request.adoc[]

[[error-code-table]]
=== 에러코드 테이블

전체 에러코드는 다음 표에서 확인할 수 있습니다.
해당 표에서 확인되지 않는 에러코드는 별도 문의 바랍니다.

[[error-code-aar-table]]
==== 소리기록실 에러코드

include::{snippets}/aar-error-code/error-code-response-fields.adoc[]

[[error-code-admin-table]]
==== 관리자 에러코드
include::{snippets}/admin-error-code/error-code-response-fields.adoc[]

[[error-code-file-table]]
==== 파일 에러코드

include::{snippets}/file-error-code/error-code-response-fields.adoc[]
