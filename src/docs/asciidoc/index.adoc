...
> 위 요청에서 각 이미지 파일의 **Form 태그 name 속성값**은 아래 <<Metadata>>와 일치해야 합니다.

===== Metadata
include::{snippets}/admin-house-get-metadata/response-fields.adoc[]

===== Metadata 예시
include::{snippets}/admin-house-get-metadata/response-body.adoc[]

operation::admin-house-post[snippets='http-response,response-fields']

[[admin-house-read-list-api]]
==== 하우스 리스트 조회 API
...
[[file-private-images-download-api]]
==== 이미지 로드 API
...
[[file-private-audios-download-api]]
==== 음원 로드 API
